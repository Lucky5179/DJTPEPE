(() => {
  ...
  let r = (t, e = !1, o = 500, c = 0) => {
    ...
    if (a) {
      ...
      s(`[gotoBlock]: Юхуу...їдемо до ${t}`);
    } else s(`[gotoBlock]: Йой... Такого блоку немає на сторінці: ${t}`);
  };
  t.watcher = new (class {
    constructor(t) {
    ...
    scrollWatcherLogging(t) {
      this.config.logging && s(`[Спостерігач]: ${t}`);
    }
  ...
  let h;
  function g() {
    (h = window.innerWidth < 1630 ? 15 : (window.innerWidth - 1600) / 2),
      d.length &&
        d.forEach((t) => {
          t.style.paddingLeft = h + "px";
        }),
      u.length &&
        u.forEach((t) => {
          t.style.paddingRight = h + "px";
        });
  }
  g(), (document.body.onresize = g);
  const m = document.querySelector(".vote__button"),
    f = document.querySelector(".vote__box"),
    p = document.querySelectorAll(".vote__content-item"),
    w = document.querySelector(".vote__content-total-value");
  window.onload = () => {
    function t() {
      const t = parseInt(w.innerText);
      if (t)
        for (let e = 0; e < p.length; e++) {
          const o = p[e].querySelector(".vote__content-item-quantity");
          let c;
          switch (e) {
            case 0:
              c = 0.4;
          ...
          o.innerText = (t * c).toFixed(0);
        }
    }
    function e() {
      let e = parseInt(w.innerText);
      e &&
        ((e += parseInt(Math.floor(6 * Math.random() + 1))),
        (w.innerText = e.toFixed(0)),
        localStorage.setItem("totalValue", e),
        t());
    }
    localStorage.getItem("totalValue") &&
      (w.innerText = "" + localStorage.getItem("totalValue")),
      t(),
      setInterval(() => {
        e();
      }, 4e3),
      (m.onclick = () => {
        f.classList.contains("active") ||
          (!(function () {
            let t = parseInt(w.innerText);
            (t += 1),
              (w.innerText = t.toFixed(0)),
              localStorage.setItem("totalValue", t);
          })(),
          t(),
  ...
  const y = document.getElementById("play-btn"),
    v = document.getElementById("prev-btn"),
    L = document.getElementById("next-btn"),
    S = document.querySelectorAll(".how__player-controlls-audio");
  let b = 0,
    E = 0;
  async function W() {
    try {
      const t = "0x79ab319c02474ff9f162a889ed1b9bb78273765a";
  ...
  (y.onclick = () => {
    y.classList.contains("play")
      ? (y.classList.remove("play"), S[b].pause(), (E = 0))
      : (y.classList.add("play"), S[b].play(), (E = 1));
  }),
    (v.onclick = () => {
      E && (y.classList.remove("play"), S[b].pause()),
        0 === b ? (b = 4) : (b -= 1),
        (S[b].currentTime = 0),
        E && (y.classList.add("play"), S[b].play());
    }),
    (L.onclick = () => {
      E && (y.classList.remove("play"), S[b].pause()),
        4 === b ? (b = 0) : (b += 1),
    ...
    document.querySelector(".icon-menu") &&
      document.addEventListener("click", function (t) {
        o &&
          ...
          (((t = 500) => {
            document.documentElement.classList.contains("lock") ? c(t) : a(t);
          })(),
        ...
        if ("click" === e.type) {
          ...
          if (o.closest("[data-goto]")) {
            const c = o.closest("[data-goto]"),
              a = c.dataset.goto ? c.dataset.goto : "",
              s = !!c.hasAttribute("data-goto-header"),
